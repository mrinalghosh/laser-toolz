...
# glyph mode font packs (one-time): fetches the bundled OFL faces into fonts/.
# The three small ones are committed; this pulls Unifont (~5MB, gitignored).
...
- **glyph** — ASCII / Unicode line art: a grid of glyphs whose ink density tracks tone. Needs `freetype-py`, which hands over each glyph's **native font outline** (drawn as hairlines — no tracing) plus a rasterizer reused to rank glyphs by ink coverage and tag each one's dominant stroke orientation. Cells pick by coverage (`--glyph-palette` sorted light→dark); with `--glyph-edge`, cells on a coherent edge instead pick the directional glyph whose stroke aligns with it (reuses the flow structure-tensor math). Palettes: `ascii` / `blocks` / `boxdraw` / `favorites` (bundled from a glyph-archive export), or import any [glyph-archive](https://mrinalghosh.github.io/glyph-archive/) JSON via `--glyph-json` (CLI) / the Import button (web). **Font packs** (`_GLYPH_PACKS`, selected by `--glyph-pack` / the web dropdown) are named freetype stacks — `system` · `sans` · `mono` · `serif` · `unifont` — where the pack's *primary* font styles the common glyphs and a shared `_GLYPH_FALLBACKS` tail (bundled Noto Sans Symbols 2 + macOS Apple Symbols / STIX Two Math / Hiragino) covers exotic symbol / arrow / alchemical / CJK-description codepoints so imported sets render instead of dropping — exactly the coverage the glyph-archive *web* app gets by streaming Noto Sans Symbols 2 from Google Fonts, but freetype needs the files on disk (bundled in `fonts/`, run `fetch_fonts.py`; the `unifont` pack is GNU Unifont's near-total-BMP blocky-pixel look). Coverage is honest: `image_to_svg` reports `glyphs_requested`/`glyphs_missing` for a glyph render (via `glyph_coverage`), the CLI warns on stderr when any drop, and the web UI shows a live "✓ all N" / "⚠ N unsupported" badge next to the pack. `--glyph-font` still forces a single font file (wins over `--glyph-pack`; stats then label the pack `custom`). `--glyph-instance` is the one exception to the "modes return flat mm polylines → `polylines_to_svg`" contract: it returns a `GlyphInstances` (distinct glyph outlines + per-cell placements) that `image_to_svg` routes to `glyph_instances_to_svg`, emitting `<defs>` + one `<use transform="translate">` per cell for a much smaller file. Uses `transform` (not `x`/`y` attributes) because that's the only form older Affinity Designer's importer honors. **Density metric** (`--glyph-density`, `_glyph_density_key`): the light→dark ramp is ranked by either `coverage` (rasterized ink area, the default/legacy) or `outline` (total stroke *length* of the glyph's own contours ≈ microperforation count) — outline is the laser-honest order because a hairline cut only traces the outline, so a solid `█` cuts as a near-empty box and must sort *light*, not dark. `_analyze_glyph` records both `cov` and `length`; the metric feeds render *and* the palette sheet identically. **Palette sheet export** (`glyph_palette_svg(p)` → `(svg, stats)`): a laser-cuttable reference swatch of the resolved charset — each glyph's native hairline outline over an etched rank index (labels drawn via `_label_polylines`, which reuses the same `_glyph_outline`/`_glyph_face` no-tracing helpers factored out of `_analyze_glyph`), sorted by `--glyph-density`, `glyph_palette_cols`×`glyph_palette_cell`mm cells. It takes **no input image** (so the CLI's `input` positional is `nargs="?"`; `main` writes it via `--glyph-palette-export PATH` and returns before requiring an image), reuses `polylines_to_svg`, and works for built-in / explicit / imported charsets alike. Web UI: a `/glyph_palette` GET download endpoint (named `<palette|imported>_palette.svg`) plus a Density-order dropdown (live) and an Export-palette button in the glyph panel. The three new `Params` fields (`glyph_density` str, `glyph_palette_cols` int, `glyph_palette_cell` float) are registered in server's `_STR_FIELDS`/`_INT_FIELDS` per the type-map rule.
...
**[segment.py](segment.py)** is a self-contained CLI, NOT a linify mode, because its output is filled multi-colour regions — the exact opposite of the laser hairline invariant. It mirrors linify's patterns: a single `SegParams` dataclass holds all tunables; `image_to_segmentation_svg(source, p)` is the public API returning `(svg, stats)`. Pipeline: `load_rgb` → MobileSAM `SamAutomaticMaskGenerator` (`generate_masks`, sorted large→small, area-filtered) → `_dedup_masks` (drop nested duplicates by IoU, since SAM emits a whole shape *and* its sub-parts) → `mask_to_rings` (cv2 `findContours` with `RETR_CCOMP`, holes rendered via `fill-rule="evenodd"`, decimated with linify's `rdp`) → `regions_to_svg` (one labelled `<path>` per region, Inkscape `inkscape:label`/layer attrs). It **imports linify's mm-grid encoders** (`_num`/`_pair`/`rdp`/`_MM_PER_UNIT`) so both tools share the true-scale coordinate convention, but has its own SVG document wrapper (linify's `_svg_document` hardcodes `fill:none`). Weights live at `weights/mobile_sam.pt` (the whole `weights/` dir is gitignored, ~40 MB) and are **auto-downloaded on first use** by `ensure_checkpoint` — a small stdlib-`urllib` helper that streams the checkpoint from the MobileSAM repo to a `.part` sibling and atomically renames on success (idempotent no-op once present); `generate_masks` calls it, and `segment_server.py` imports and calls it too, so neither CLI nor web UI needs a manual `curl`. Runs CPU-only: the automatic generator feeds float64 points that Apple MPS rejects, so MPS is not auto-picked.
